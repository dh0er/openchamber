...
'settings.remoteInstances.clientAuth.actions.addDevice': '添加设备',
  ...
  'settings.providers.page.connect.noProvidersAvailable': '没有可用的提供商。',
  'settings.providers.page.connect.instanceNameLabel': '实例名称',
  'settings.providers.page.connect.instanceNameTooltip': '显示在 OpenChamber 的各个位置。更改名称不会改变稳定的提供商 ID。',
  ...
  'settings.providers.page.auth.apiKeyTooltip': '密钥存储在 OpenCode 的凭据存储中，而不是 OpenChamber 设置中。',
  'settings.providers.page.auth.apiKeyPlaceholder': 'sk-...',
  'settings.providers.page.auth.apiKeyExistingPlaceholder': '留空以保留当前密钥',
  'settings.providers.page.auth.baseUrlLabel': '基础 URL',
  'settings.providers.page.auth.baseUrlRequiredLabel': '基础 URL（必填）',
  'settings.providers.page.auth.baseUrlTooltip': '用于自定义网关的可选 HTTP(S) API 端点。',
  'settings.providers.page.auth.openaiCompatibleBaseUrlTooltip': 'OpenAI 兼容网关所需的 API 根地址。OpenChamber 会从 /models 加载可用模型。',
  'settings.providers.page.auth.baseUrlPlaceholder': 'https://gateway.example.com/v1',
  'settings.providers.page.auth.proxyModeLabel': '代理',
  'settings.providers.page.auth.proxyModeTooltip': '仅应用于此托管 API 密钥实例。Windows 使用系统代理设置（包括 PAC）；macOS/Linux 使用 HTTP(S)_PROXY 和 NO_PROXY。',
  'settings.providers.page.auth.proxyModeDirect': '直连',
  'settings.providers.page.auth.proxyModeSystem': '系统 / 环境（Windows PAC）',
  'settings.providers.page.auth.proxyModeManual': '手动',
  'settings.providers.page.auth.proxyUrlLabel': '代理 URL',
  'settings.providers.page.auth.proxyUrlTooltip': '代理的 HTTP(S) 源：仅包含协议、主机和可选端口。不允许包含凭据和 URL 路径。',
  'settings.providers.page.auth.proxyUrlPlaceholder': 'http://127.0.0.1:9000',
  'settings.providers.page.auth.subscriptionAlreadyConnected': '此提供商已连接一个订阅。你仍可添加另一个使用 API 密钥的实例。',
  'settings.providers.page.auth.apiKeyWillBeReplacedBySubscription': '连接订阅将替换此提供商当前的 API 密钥凭据。如果要同时保留两者，请先添加一个单独的 API 密钥实例。',
  'settings.providers.page.auth.subscriptionConnection': '订阅连接',
  ...
  'settings.providers.page.actions.connectInstance': '连接实例',
  'settings.providers.page.actions.saveConnection': '保存连接',
  ...
  'settings.providers.page.toast.instanceNameRequired': '实例名称为必填项',
  'settings.providers.page.toast.baseUrlInvalid': '请输入不含凭据的有效 HTTP(S) 基础 URL',
  'settings.providers.page.toast.baseUrlRequired': 'OpenAI Compatible 必须填写基础 URL',
  'settings.providers.page.toast.proxyUrlRequired': '手动代理设置必须填写代理 URL',
  'settings.providers.page.toast.proxyUrlInvalid': '请输入不含凭据、路径、查询或片段的 HTTP(S) 代理源',
  'settings.providers.page.toast.modelsLoadFailed': '无法从此 OpenAI 兼容网关加载模型。请检查基础 URL 和 API 密钥。',
  'settings.providers.page.toast.connectionSaveFailed': '无法保存提供商连接',
  'settings.providers.page.toast.instanceConnected': '提供商实例已连接',
  'settings.providers.page.toast.connectionSaved': '提供商连接已保存',
  'settings.providers.page.toast.savedReloadFailed': '提供商连接已保存，但 OpenCode 重载失败。',
  ...
  'settings.providers.page.toast.oauthConnectedSettingsSaveFailed': '订阅已连接，但无法保存其连接设置。',
