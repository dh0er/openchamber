...
import { readAuthFile, writeAuthFile } from './opencodeAuth';

const temporaryDirectories = [];

const createTemporaryAuthPath = () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'openchamber-vscode-auth-'));
...
describe('VS Code OpenCode auth storage', () => {
  test('atomically creates private auth JSON in the target directory', () => {
  ...
  test('keeps the original file and cleans the private temp file when rename fails', () => {
    const { directory, authFile } = createTemporaryAuthPath();
    ...
    const errorCalls = [];
    console.error = mock((...args) => { errorCalls.push(args); });
    ...
    expect(JSON.stringify(errorCalls)).not.toContain('new-secret-must-not-log');
    if (process.platform !== 'win32') {
      expect(fs.statSync(authFile).mode & 0o777).toBe(0o600);
  ...
  test('fails closed and removes a backup when POSIX chmod cannot secure it', () => {
    const { directory, authFile } = createTemporaryAuthPath();
    const original = '{"openai":{"type":"oauth"}}\n';
    ...
    expect(fs.readdirSync(directory).filter((entry) => entry.endsWith('.tmp'))).toEqual([]);
    expect(JSON.stringify(errorCalls)).not.toContain('chmod-secret-must-not-log');
  });
});
